...
/**
 * 指定なしの場合のウィンドウサイズ(unit)
 * @type {number[]}
 */
nas.GUI.defaultWinsize = [8, 6];
/**
 * 指定なしの場合のウインドウ初期位置(px)
 * @type {number[]}
 */
nas.GUI.dafaultOffset = [240, 40];
/**
 * ウインドウタイトルデフォルト
 * @type {string}
 */
nas.GUI.defaultName = "No Name";
 ...
 * @type {number}
 */
nas.GUI.workBase.maxLength = 4;
/**
 * 現在のフォルダ
 * @type {number}
 */
nas.GUI.workBase.selected = 0;
/**
 * カレントを返す
 * @returns {*}
 */
...
/**
 * @desc ウインドウ作成メソッド(AE専用?ダイアログとパレットしかできません)
 */
...
nas.GUI.newWindow = function (Type, Name, width, height, offsetX, offsetY) {
    ...
    if (!Name) {
        Name = nas.GUI.defaultName;
    }
    if (!width) {
        width = nas.GUI.defaultWinsize[0];
    }
    if (!height) {
        height = nas.GUI.defaultWinsize[1];
    }
    if (!offsetX) {
        offsetX = nas.GUI.dafaultOffset[0];
    }
    if (!offsetY) {
        offsetY = nas.GUI.dafaultOffset[1];
    }
...
if (nas.GUI.UIlvl > 1) {
    ...
    nas.GUI.newImage = function (myFile) {
        ...
        if (false) {
            /**
             * はやくサポートして判定式を書かせてほしい
             * @type {nas.GUI.newImage}
             */
            var myImage = new ScriptUI.newImage(myFile);
        } else {
            if (!nas.GUI.stabWindow) {
                nas.GUI.stabWindow = new Window("palette", "nasGUIStab", [0, 0, 100, 100])
            }
            /**
             *  なかったら仮ウィンドウ作る
             */
            if ((!myFile) || (!(myFile.exists)) || (!(myFile.name.match(/\.png$/i)))) {
                myFile = new File(Folder.nas.path + "/nas/lib/resource/Icons/default.png")
            }
...
//	var myIconButton=Parent.add("iconbutton",[100,100,132,132],myFile,{style:"toolbutton"});
    ...
    nas.GUI.getIcon = function (myFile) {
        if ((!myFile) || (!(myFile.exists))) {
            return this.systemIcons["default"]
        }
        if (myFile.name.match(/(.+)\.([^\.]+)$/)) {
            var myFileName = RegExp.$1;
            var myFileExt = RegExp.$2;
        } else {
...
//		alert(myFile.path+"/"+myFileName+".png")
    ...
    /**
     * アイコンボタンが必要な局面でも動作を継続させるために
     * ダミーの画像オブジェクトや代用ボタンなどを作ればAE7以前でも何がしか動作するかも
     * まだ未対応(2010 0308)
     *
    ...
    nas.GUI.newImage = function () {
        return false
    };
    nas.GUI.getIcon = function () {
        return false
    };
    nas.GUI.addIconButton = function (Parent, Label, left, top, width, height, myFile) {
        return this.addButton(Parent, Label, left, top, width, height);
    };//ラップしておく
...
nas.GUI.addScrollBar = function (Parent, Value, minValue, maxValue, left, top, height, align) {
    ...
    switch (align) {
        case    "center"    :
            var alignOffset = (this.colUnit - this.lineUnit * .7) / 2 - this.leftPadding;
            break;
        case    "left"    :
            var alignOffset = 0;
            break;
...
nas.GUI.addSelectButton = function (Parent, Label, Select, left, top, width, height) {
    /**
     * AE7以前は元のコード　以降はネイティブのドロップダウンリストを使用するように
     * 改装しようかと思ったけどメソッド互換があまりに低いのでヤメ
     * かわりに　selectOptions　ドロップダウンリスト版を作成しようそうしよう
     *
     * @type {boolean}
     */
...
nas.GUI.addDropDownList = function (Parent, Options, Select, left, top, width, height, props) {
     ...
     *  add items
     */
    for (var itmIdx = 0; itmIdx < Options.length; itmIdx++) {
        myDDL.add("item", Options[itmIdx]);
    }
    ...
    if (Selected.length > 0) {
        for (var slIdx = (Selected.length - 1); slIdx >= 0; slIdx--) {
            for (itmIdx = 0; itmIdx < myLB.items.length; itmIdx++) {
                if (myLB.items[itmIdx].index == Selected[slIdx]) {
                    myLB.items[itmIdx].selected = true;
                }
            }
        }
    }
...
nas.GUI.addListBox = function (Parent, Options, Selected, left, top, width, height, option) {
     ...
     * @type {boolean}
     */
    myLB.multiSelect = (option == "multiselect");
    myLB.editable = (option == "editable");
    /**
     * 表示最大高さ
     */
    myLB.DisplayHeight = height;
    /**
     * ラベルテキスト(編集可能な場合のみ作成)
     */
    if (myLB.editable) {
    ...
    myLB.select = function (myIndex) {
        if ((myIndex >= 0) && (myIndex < this.options.length)) {
            ...
            for (var idx = 0; idx < this.options.length; idx++) {
                if (this.multiSelect) {
                    if (myIndex == idx) {
                        newSelected[idx] = (this.checks[idx]) ? false : true
                    } else {
                        newSelected[idx] = this.checks[idx]
                    }//multi
                } else {
                    if (myIndex == idx) {
                        newSelected[idx] = (this.checks[idx]) ? false : true
...
//			this.listBox.items[n].selected=this.checks[n]
            ...
            if (this.checks.length != this.listBox.items.length) {
                alert("err")
            }
            /**
             * value初期値設定
                ...
                if (myLB.editable) {
                    ...
                    if (this.value.length == 0) {
                        ...
                        if (this.value.length == 1) {
                            this.labelText.text = this.value[0];
                        } else {
...
//		myLB.ChgRange.viewOffset=0;//スクロールバーのvalueを使用
        ...
        myLB.setOptions = function (newOptions, newSelected) {
            ...
            for (n = 0; n < this.DisplayHeight; n++) {//チェックボックス表示
...
//		this.parent.display[idx].text="\[ "+id.toString()+" \] "+this.parent.options[id];
        ...
        myLB.check = function (id) {
            if (this.multiSelect) {//マルチセレクト
                 ...
                 * オプションスキャン
                 */
                for (var idx = 0; idx < this.options.length; idx++) {//対象外を更新
                    if (idx != optId) {
                        this.checks[idx] = false
                    }
                }
                for (var idx = 0; idx < this.DisplayLines; idx++) {//表示更新
...
//第一引数が配列でない、または配列であっても要素数が1の場合はfalseを返して終了
        ...
        /**
         * 引数の事前処理
         * 第三引数がButton/Panel/EditTextオブジェクトだった場合は、
         * GUIオブジェクトから位置と幅を取得
         * その場合、第四引数が表示行数
         */
        ...
        if (isNaN(selectedId) || (selectedId >= options.length) || (selectedId < 0)) {
            selectedId = null;
        }
        /**
        ...
        if (nas.GUI.optionSelector) {
            if (nas.GUI.optionSelector.visible) {
                nas.GUI.optionSelector.close();
            }
...
//	nas.GUI.optionSelector.selector=new Array();//セレクトボタン格納配列(後で初期化する)
        nas.GUI.optionSelector.init = function () {
            ...
            for (var idx = 0; idx < this.displayLines; idx++) {
                ...
                this.selector[idx].onClick = function () {
...
//	nas.GUI.optionSelector.text=nas.GUI.optionSelector.options[nas.GUI.optionSelector.selected].toString();
        nas.GUI.optionSelector.onShow = function () {
            this.selector[0].active = true;
        };
...
//	nas.GUI.optionSelector.selector=new Array();//セレクトボタン格納配列(後で初期化する)
        nas.GUI.optionSelector.init = function () {
            ...
            if (newUI) {
                ...
                if (this.selected) {
                    this.selector.items[this.selected].selected = true
                }
                this.selector.onChange = function () {
                    this.parent.selected = this.selection.index;
                    this.parent.close();
                };
...
//	nas.GUI.optionSelector.text=nas.GUI.optionSelector.options[nas.GUI.optionSelector.selected].toString();
        ...
        if (!(newUI))    nas.GUI.optionSelector.onShow = function () {
            this.selector[0].active = true;
        };
        if (left < 0 || top < 0)nas.GUI.optionSelector.center();
        nas.GUI.optionSelector.show();
...
//	var myCB =nas.GUI.addPanel(Parent,"",left,top,width,height+0.4);//必ずtextは""で初期化
    ...
    /**
     * ライブラリを使用するとパネル用オフセットが乗るので、
     * 補正値をのせるかまたは直接メソッドコールをすること
     * オブジェクトプロパティ
     */
...
//			if((! height)||(height<(2*dim+1))){height=(2*dim+1)};//最小サイズあり
    ...
    if (!defaultValue) {
        defaultValue = defValue[type][1]
    }
    if (!minValue) {
        minValue = defValue[type][2]
    }
    if (!maxValue) {
        maxValue = defValue[type][3]
    }
    /**
     * トレーラ（パネル）
     * @type {*|Array}
     ...
     * @type {number}
     */
    var shiftL = (type == "angle") ? 1 : 0;
    var spanT = (type == "angle") ? 2 : 1;
    if (dim > 1) {
        myMC.labelText = [];
        myMC.valueText = [];
        myMC.valueSlider = [];
        ...
        for (var idx = 0; idx < dim; idx++) {
            ...
            myMC.valueText[idx].onChange = function () {
                if ((this.text != this.parent.value[this.index]) && (!(isNaN(this.text)))) {
                    ...
                    if (this.parent.isReg) {
                        if (myValue < this.parent.minValue[this.index]) {
                            myValue = this.parent.minValue[this.index]
                        }
                        if (myValue > this.parent.maxValue[this.index]) {
                            myValue = this.parent.maxValue[this.index]
                        }
                    }
                    this.parent.set(myValue, this.index);
                } else {
                    this.text = this.parent.value[this.index].toString()
                }
            };
            /**
             * 値スライダ gamma指定時は対数動作
            ...
            if (type == "gamma") {
                ...
                myMC.valueSlider[idx].onChanging = function () {
                    ...
                    if (myGamma > this.parent.maxValue[this.index]) {
                        this.value = this.maxvalue;
                    }
                    if (myGamma < this.parent.minValue[this.index]) {
                        this.value = this.minvalue;
                    }
                    if (myGamma != this.parent.value[this.index]) {
                        this.parent.set(myGamma, this.index, true)
                    }
                };
                myMC.valueSlider[idx].onChange = function () {
                    ...
                    if (myGamma != this.parent.value[this.index]) {
                        this.parent.set(myGamma, this.index, skip = true)
                    }
                }
            } else {
                /**
                ...
                myMC.valueSlider[idx].index = idx;
                myMC.valueSlider[idx].onChanging = function () {
                    if (this.value != this.parent.value[this.index]) {
                        this.parent.set(this.value, this.index, skip = true)
                    }
                };
                myMC.valueSlider[idx].onChange = function () {
        ...
        myMC.valueText.onChange = function () {
            if ((this.text != this.parent.value) && (!(isNaN(this.text)))) {
                var myValue = this.text * 1;
                if (this.parent.isReg) {
                    if (myValue < this.parent.minValue) {
                        myValue = this.parent.minValue
                    }
                    if (myValue > this.parent.maxValue) {
                        myValue = this.parent.maxValue
                    }
                }
                this.parent.set(myValue);
            } else {
                this.text = this.parent.value.toString()
            }
        };
        /**
         * 値スライダ gamma指定時は対数動作
        ...
        if (type == "gamma") {
            ...
            myMC.valueSlider.onChanging = function () {
                ...
                if (myGamma > this.parent.maxValue) {
                    this.value = this.maxvalue;
                }
                if (myGamma < this.parent.minValue) {
                    this.value = this.minvalue;
                }
                if (myGamma != this.parent.value) {
                    this.parent.set(myGamma, false, skip = true)
                }
            };
            ...
            myMC.valueSlider.onChanging = function () {
                if (this.value != this.parent.value) {
                    this.parent.set(this.value, false, skip = true)
                }
            };
            myMC.valueSlider.onChange = function () {
    ...
    myMC.set = function (newValue, myIndex, skip) {
        if (this.dim > 1) {
                ...
                if (!(isNaN(newValue))) {
                    ...
                    if (this.isReg) {
                        if (this.valueType == "angle") {
                            ...
                            if (newValue < this.minValue[myIndex]) {
                                newValue = this.minValue[myIndex]
                            }
                            if (newValue > this.maxValue[myIndex]) {
                                newValue = this.maxValue[myIndex]
                            }
                        }
                    }
                    ...
                    if (this.valueText[myIndex].text != newValue) {
                        this.valueText[myIndex].text = newValue
                    }
                    if (this.valueType == "gamma") {
                        var mySlider = (this.value[myIndex] * 10000) / (1 + this.value[myIndex]);
                        if (this.valueSlider[myIndex].value != mySlider) {
                            this.valueSlider[myIndex].value = mySlider
                        }
                    } else {
                        if (this.valueSlider[myIndex].value != newValue) {
                            this.valueSlider[myIndex].value = this.value[myIndex]
                        }
                    }
                    /**
                     * 角度アイコンありの時のみ
                    ...
                    if (skip) {
                        /**
                         * メソッドの最後にonChangingをコール
                         */
                        this.onChanging();
                    } else {
                        /**
                         * メソッドの最後にonChangeをコール
                         */
                        this.onChange();
                    }
                }
            ...
            if (!(isNaN(newValue))) {
                ...
                if (this.valueText.text != newValue) {
                    this.valueText.text = newValue
                }
                if (this.valueType == "gamma") {
                    var mySlider = (this.value * 10000) / (1 + this.value);
                    if (this.valueSlider.value != mySlider) {
                        this.valueSlider.value = mySlider
                    }
                } else {
                    if (this.valueSlider.value != newValue) {
...
nas.GUI.addSelectSwitch = function (Parent, Label, Select, left, top, width, height, icons) {
    /**
     * 省スペースUIとして以前のバージョンのセレクトボタンと
     * 同等のコントロールを復活セレクタ内のオプションをクリックのたびに切りかえる
     * アイコン対応
     * @type {boolean}
     */
    var newUI = (AEVersion >= 8);
    if ((newUI) && (icons)) {
        ...
        for (var idx = 0; idx < mySS.options.length; idx++) {
            if (icons.length > idx) {
                mySS.optionIcons[idx] = icons[idx];
            } else {
                mySS.optionIcons[idx] = null;
            }//あれば取得なければnull
