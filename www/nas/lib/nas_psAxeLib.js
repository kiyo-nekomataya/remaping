/*(nas_psAxeLib.js)
 *	photoshop用アニメーション拡張ライブラリ PhotoShop Animation eXtEntion
 *	アイコンは斧だ！ ざっくり ばっさり行こう
 *	UXP環境対応改装
 *	nas_psAxeLib.js 2022/02/28  kiyo
 * UXPのコーディングスタイルが新しいので、ソース内に混在するとESTK環境で障害が発生する
 * このファイルではESTKのコーディングスタイルを保持して、
 * 必要な部分は別のソースからの読み出しを行うように変更 220625
 */
/**
 *	@fileoverview
 *	Object nas 配下にphotoshopスクリプト環境のためのアニメーション作業用拡張オブジェクトを設定する
 *	AEの nas.otome に相当するオブジェクト
 */
/**
 *	@class
 */
if(typeof nas.axe == 'undefined') nas.axe = {};
//grobal
	var moduleName="psAxeLib"
	var myFilename=("nas_psAxeLib.js")
		var myFilerevision=("2.1");//2021 01.03
	if(nas.Version){
		nas.Version[moduleName] = moduleName+" :"+myFilename+" :"+myFilerevision;
	};
//システムオブジェクトに拡張プロパティを増設
/*
	UXP 環境では Object Folder が設定されない
	仮のオブジェクトとして初期化して パスを保存する
*/
	if((appHost.platform == 'UXP')&&(appHost.uxp_host.name == 'Photoshop')){
...
const action = require('photoshop').action;
		...
		Folder.nas     = new Folder(nas.baseLocation.fullName);
		Folder.scripts = app.path + "/" + localize("$$$/ScriptingSupport/InstalledScripts=Presets/Scripts");
	};

//ローカライズパスを取得
//AE用プロパティ AE/PS同時使用の際にバッティングするので、ダミーオブジェクトとして初期化 実際には使用されない AEサイドでも同様の処置が必要
...
//	ドキュメントマネージャ関連
	...
	nas.axe.dmCurrent  = [0,10,"s-c012",72] ;//最後に操作したドキュメント情報[タイトルDBid,opusNo.,cutNo.,time]
...
if(nas.locale=="ja"){
			...
			nas.paperSizes.push("KC-B4横(364.2x257.2)",[364.2,257.2]);
			...
			nas.paperSizes.push("KC-jisB4landscape(364.2x257.2)",[364.2,257.2]);
...
if(nas.locale=="ja"){
			nas.registerMarks.push("3穴トンボ",["peg3p1.svg"]);//0
			nas.registerMarks.push("3穴白抜き",["peg3p2.svg"]);//1
			nas.registerMarks.push("3穴ベタ",["peg3p3.svg"]);//2
			nas.registerMarks.push("3穴外黒",["peg3p4.svg"]);//2
			nas.registerMarks.push("2穴トンボ",["peg2p1.svg"]);//3
			nas.registerMarks.push("2穴白抜き",["peg2p2.svg"]);//4
			nas.registerMarks.push("2穴ベタ",["peg2p3.svg"]);//5
}else{
			nas.registerMarks.push("ACMECloss",["peg3p1.svg"]);//0
			nas.registerMarks.push("ACMEOutline",["peg3p2.svg"]);//1
			nas.registerMarks.push("ACMEShape",["peg3p3.svg"]);//2
			nas.registerMarks.push("ACMEInverse",["peg3p4.svg"]);//2
			nas.registerMarks.push("2holeCloss",["peg2p1.svg"]);//3
			nas.registerMarks.push("2holeACMEOutline",["peg2p2.svg"]);//4
			nas.registerMarks.push("2holeShape",["peg2p3.svg"]);//5
};
//	作画(ソース)データの標準フレーム
...
*/
	...
	nas.inputMedias=new nas.nTable();
		nas.inputMedias.onChange=function(){
//メディアがセレクトされたらシステムの解像度とフレームレートを変更する
//解像度(DPC)・フレームレート(FPS)をオブジェクト化
				var currentResolution = new nas.UnitResolution(this.selectedRecord[3],'dpi');
				var currentRate       = new nas.Framerate(this.selectedRecord[4]);
				...
				if(nas.RESOLUTION.as("dpi")!=currentResolution.as('dpi')) nas.RESOLUTION.setValue(this.selectedRecord[3],'dpi');
				if(!(nas.FRATE.sameAs(currentRate))) nas.FRATE.parse(this.selectedRecord[4]);
...
*/
	nas.outputMedias=new nas.nTable();
...
if(true){
			...
			nas.workTitles.push("(long title name)",["(short name)","(prefix)",6,4]);
...
/*
	Adobe系列の旧形式WorkTitlesと新しいpmdb形式のwotkTitlesを同期
	pmdb形式のほうが主体なので旧形式のBDを排除してpmdb側の情報で上書きする
	更新時はpmdbを自動で更新する
photoshop のimはメディアDBでなく AnimationFieldに近似しているのでそちらから生成したほうが良い
2024
			nas.inputMedias.push(
				"254mm/16:9/200dpi",[   ;//name = [width +'mm',aspect,resolution].join('/')
					254,                ;//Number width as mm
					"16/9",             ;//String frame aspect
					200,                ;//Number resolution
					24,                 ;//Number framerate
					2,                  ;//Int pegform id
					0,                  ;//
					105,                ;//
...
*/
	for (var mdx in nas.pmdb.medias.members){
		var mdd = nas.pmdb.medias.entry(mdx);
		if(mdd.mediaType == "drawing"){
			var afld = nas.StanderdAnimationFields[mmd.animationField]
			if(afld){
				nas.inputMedias.push(
					key-string,[
						new nas.UnitValue(afld.baseWidth).as('mm'),
						mdd.frameAspect,
						new nas.UnitResolution(mdd.baseResolution).as('dpi'),
						new nas.Framerate(mdd.framerate.toString()).rate,
						mdd.pegForm.name,
						mdd.pegOffset.x.as('mm'),
						mdd.pegOffset.y.as('mm'),0
//						mdd.animationField.pegOffset.r.as('degrees')
					]
				);
			}
			...
			nas.inputMedias.push(
				mdd.mediaName,[
					new nas.UnitValue(mdd.baseWidth).as('mm'),
					mdd.frameAspect,
					new nas.UnitResolution(mdd.baseResolution).as('dpi'),
					new nas.Framerate(mdd.framerate.toString()).rate,
					mdd.pegForm.name,
...
//					mdd.animationField.pegOffset.r.as('degrees')
			...
			(mdd.mediaType == "movie")||(mdd.mediaType == "video")
		){
			var width = Math.round(new nas.UnitValue(mdd.baseWidth).as('in')*new nas.UnitResolution(mdd.baseResolution).as('ppi'));
			var height = Math.round(width/mdd.frameAspect);
			nas.outputMedias.push(
				mdd.mediaName,[
					width,
					height, 
					new nas.Framerate(mdd.framerate).rate,
					mdd.pixelAspect
				]
			);
		};
	};
	var titles = JSON.parse(nas.pmdb.workTitles.dump('JSON'));
	nas.wortTitles = new nTable();
	for (var name in nas.pmdb.workTitles.members){
		var title = nas.pmdb.workTitles.members[name];
		nas.wortTitles.push(
			title.fullName,[
				title.shortName,
				title.code,
...
/*pmdbからデータを取得してaxe|otome形式のデータを作成する*/
};
...
//レイヤー（アセット）名調整変数
nas.axe.numDigits  = 1;//デフォルト連番桁数
nas.axe.nameExt    = false;//マルチネーム拡張無し
nas.axe.numOrderUp = true;//下から順
...
//以下拡張メソッドトレーラークラス アニメーションフレーム操作関数などが多い nas.axeAFC,VTC,CMC
	nas.axeAFC = {};//フレームタイムライン操作オブジェクトトレーラー
	nas.axeVTC = {};//ビデオタイムライン操作オブジェクトトレーラー
	nas.axeCMC = {};//一般オブジェクト操作
...
//	nas.axeAFC = new Object;//アニメウインドウ操作関数 現状取得ができないのはヘボいが今のトコはカンベン 後で整理する
/*
...
: nas.axeCMC.layersSetName(nameString,postfix,ctrl) pman互換のレイヤーリネーム
...
nas.axeCMC.getSelectedLayers = function(expandOption){
	var resultLayers = nas.axeCMC.selectItemsById(nas.axeCMC.getSelectedItemId());
	if(expandOption) return resultLayers.filter(function(e){return (e instanceof ArtLayer)});
	return resultLayers;
}
//=======================　セレクト状態のアイテムID取得
...
nas.axeCMC.getAllItems =function(myTrailer){
	...
	if((myTrailer.typename == "LayerSet")||(myTrailer.typename == "Document")) myTrailer = myTrailer.layers;
...
nas.axeCMC.selectItemsById=function(idx){
   ...
   for( var i = 0; i < idx.length; i++ ){
      ...
      executeAction( charIDToTypeID( "slct" ), desc, DialogModes.NO );
      result.add(app.activeDocument.activeLayer);
   };
   return result;
};
// =================== UNDOバッファを使用して復帰
 ...
 *	@params {Boolean}	ctrl
 		マルチネーム操作オプション
 * UAToolboxの機能をpsAXeへ移植 220622
 item.name > layer.name
 item.text > layer,name

 *  レイヤ名設定
 *  引数文字列の指定で選択レイヤの名前を操作
 *  標準機能は引数文字列をグループプリフィックスとして連番化する
 *  選択されたアイテムの親グループ内で名称がユニークにならない場合も操作をリジェクトしない（ここはファイル名ではないので仕様が異なる）
 *  与えられるナンバーは整数で増加ステップは１固定
 *  既存名のグループが存在すれば、次点の番号を補完する（[2,3,5]に対して 6,7...を補完する 空番号の1,4は補わない）
 *  引数に空白|undefined 等のネガティブが与えられた場合 .text を .name にリセットする
 *  %は既存名と置換
 *  ++ 増番 -- 減番 ##... 桁あわせ
 *  ポストフィックスオプションがあれば引数をポストフィックスとして操作を行う
 *  引数がセパレータで開始する場合は、ポストフィックスオプションが指定されたものとみなす
 *  **[ctrl == true]で呼び出された場合、マルチネーム置換とシングルネーム置換を切り替える
 */
nas.axeCMC.layersSetName = function(nameString,postfix,ctrl){
//操作パラメータ確定
	...
	if((nameString.match(/^\s$/))||(! nameString)){
// reNameで行う復帰処理はシステムのUNDOで代行
// 空白の引数が与えられた場合の動作はNOP
		return;
	}else{
//操作対象のレイヤを親ごとに分類
		var selection = nas.axeCMC.getSelectedLayers().reverse();//この関数は下から順にアイテムを戻す
		if(selection.length == 0) return ;//NOP
//対象レイヤの親を一覧登録
		var renameGroup = [];
		selection.forEach(function(e){renameGroup.add(e.parent);});
//親ごとに対象レイヤーを分類
		var targetItems = [];
		renameGroup.forEach(function(j){targetItems.push( selection.filter(function(e){ return(e.parent === j);}));});
//===========グループ処理用リネームファンクションを設定
		var valueNames  = [];
		var renameGroupPostfixCount = false;
		var renameGroupCount        = false;
		if((! postfix)&&((nameString == '++')||(nameString == '--'))){
// ++|--
// 名前数値部分の増減 チェックアイテムの番号増減を行う。重複時は警告して処理失敗
// アイテム内のエレメントが複数ある場合は、[ctrl]キーで最終の一つ｜全部 を切替
			var operate = function(e){
				if(ctrl){
					var names   = e.replace(/_/g,'/').split('/');
					names.forEach(function(e,i){names[i] = nas.incrStr(e,((nameString == '++')? 1:-1),true)});
					var newName = names.join('/');
				}else{
					var newName = nas.incrStr(e,((nameString == '++')? 1:-1),true);
				};
				return newName;
			};
...
// ボディ連番部桁あわせ # 指定を
			var operate = function(e){
				var digits = nas.axe.numDigits;
				if(nameString.length > 1) digits = nameString.length;
				return nas.RZf(e,digits);
			};
		}else if(postfix){
// has param postfix ポストフィックス処理 ポストフィックスを追加｜交換する
			var postfixHeader = nameString.replace(/#/g,'');
			if(nameString.match(/#/)){
//既存グループアイテムからグループごとにポストフィックススタートナンバーを取得
				renameGroupPostfixCount = [];
				for(var h = 0 ; h < renameGroup.length ; h ++ ){
					renameGroupPostfixCount[h] = 0;
					for(var i = 0 ; i < renameGroup[h].layers.length ; i ++){
						var desc = new nas.CellDescription(renameGroup[h].layers[i].text);
						if(
							(selection.indexOf(renameGroup[h].layers[i]) < 0)&&
							(desc.postfix.indexOf(postfixHeader) == 0)
						){
							var num = nas.parseNumber(desc.postfix);
							if (renameGroupPostfixCount[h] < num) renameGroupPostfixCount[h] = num;
						};
					};
				};
			};
//関数実行前に count変数の初期化が必要
			var operate = function(e){
				var currentDescription = new nas.CellDescription(e);
				if (nameString.indexOf('#') >= 0){
					count++;
					nameString = nameString.replace(/#/,count);
				};
				return ([currentDescription.prefix,currentDescription.body,nameString]).join(sep);
			};
		}else{
//一般処理 グループプリフィックス連番
//複合エレメントをサポート
//現在のtextが_|/で終了している場合はその前方を固定文字列として扱う
			var digits = nas.axe.numDigits;
			if(nameString.match(/^(.+)([\-_])$/)){
				nameString = RegExp.$1;
				sep        = RegExp.$2;
			};
			var nameRegex = new RegExp("^"+nameString+"[\\-\\s]?([0-9]+)");
//既存アイテムからグループごとのスタートナンバーを取得
			renameGroupCount = [];
			for(var h = 0 ; h < renameGroup.length ; h ++ ){
				renameGroupCount[h] = 0;
				for(var i = 0 ; i < renameGroup[h].layers.length ; i ++){
					if(selection.indexOf(renameGroup[h].layers[i]) < 0 ){
//複合アイテムを含むグループ内全てのナンバーを検索して最大値を得る
						var subitems = renameGroup[h].layers[i].name.replace(/_/g,'/').split('/');
			...
			if( nas.axe.nameExt != (ctrl) ){
				var operate = function(e){
//チェックアイテムをサブアイテムに分解して最終アイテムをリネーム
					var subitems = e.replace(/_/g,'/').split('/');
					if (nameString.indexOf('%') == 0){
...
//サブアイテムは無視して全体をリネーム
					count++;
					if (nameString.indexOf('%') == 0) nameString = nameString.replace(/%/,e);
...
//処理メンバーを一時設定 シャローコピーをとってオーダーを調整(layersはArray.fromがエラーで動かない)
			var opMember = [];//
			for(var oi = 0 ; oi < renameGroup[k].layers.length ; oi ++) opMember.push(renameGroup[k].layers[oi]);
			if(nas.axe.numOrderUp) opMember.reverse();//オーダー反転 axe環境ではresult配列は下から順に入る
			opMember.forEach(function(e){
				var itemIx = targetItems[k].indexOf(e);
				if(itemIx < 0){
					newNames.push(e.name);
				}else{
					newNames.push(operate(e.name));
				};
			});
			if(nas.axe.numOrderUp) newNames.reverse();//オーダー反転
//PSでは、名前の衝突を検証しない
//			var checkNames = [];
//			newNames.forEach(function(e){checkNames.add(e,function(a,b){nas.compareCellIdf(a,b) >= 7});});
...
//			}else{
//コンフリクトがないので、実処理を行う
				var oprtLayers = targetItems[k][0].parent.layers;
				for(var l = 0;l < oprtLayers.length ; l++){
					if(oprtLayers[l].name != newNames[l]) oprtLayers[l].name = newNames[l];
				};
//			};
		};
	};
...
/*TEST
nas.axeCMC.layersSetName("B",false);
*/
...
try{
   ...
   if((! myOption)&&(nas.FRATE.rate != currentFR)){nas.FRATE.parse(currentFR);};
...
var mvTL=(myOffset)?false:true;//未指定または0の場合移動フラグを立てる
    ...
    var descO = new ActionDescriptor();
        ...
        descO.putDouble( stringIDToTypeID( "frameRate" ), nas.FRATE.rate);
