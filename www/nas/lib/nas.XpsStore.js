...
//		if(myIndex>myTarget.layers.length)myIndex=myTarget.layers.length;
        if (
            ...
            this.body = myTarget;
            if ((isNaN(myIndex)) || (myIndex > this.body.length)) {
    ...
    this.initBody = function () {
        ...
        /**
         * すでにあれば空コンポに初期化する。　コンポがなければ新規に作る。
         * 初期化用初期状態のシート保持数は0 選択インデックスは0
         * @type {マッチなし}
         */
        ...
        if (
            ...
            this.body = myTarget;
            for (var lIdx = 1; lIdx <= this.body.length; lIdx++) {
                this.body[lIdx].remove();
            }
        } else {
    ...
    this.select = function (index) {
         ...
         * indexのズレに注意
         */
        if ((!index) && (this.selected instanceof Xps)) {
            if (index != 0) {
...
//			this.body.layers[index].sourceText.setValue(myXps.toString().replace(/\n/g,"\\r\\n"));;
     ...
     * 直接addした場合はsetInfo忘れずに 引数なしの場合は新規Xpsを追加する
     * @param myXps
     * @returns {*}
     */
...
//			this.body.layers[index].remove();//削除する
            ...
            for (var ix = 0; ix < this.body.length; ix++) {
                this.body[ix].index = ix + 1
            }//インデックス更新
            if (this.body.length) {
                this.select(nextIndex)
            }//カレント移動（0なら解除 ＜この仕様一考の要あり）
...
//		if(appHost.version<7){return this.AE65Prop};//AE65の場合はダミープロパティを返す
    ...
    this.setInfo = function (myIndex, myFile) {
        ...
        if (this.body[myIndex - 1]) {
            ...
            /**
             * 新規プロパティを作る
             * ファイルが与えられた場合はファイルから、ない場合は現状のデータから
             */
            if (myFile) {
                var newProp = "{";
