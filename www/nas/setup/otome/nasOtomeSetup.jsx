/*	AdobeAE レンダー乙女 Setup Script 2016.06.01 Nekomataya/kiyo
新ロケーション用のAdobeAEセットアップスクリプトです
	管理者権限でのインストールが必要です
Adobe AEをターゲットホストとしてAdobeスクリプトで実行
（ダブルクリックでAdobe ESTKを立ち上げて、ホストとしてAEを選択して実行）

	このスクリプトは、以下の動作を行います。
> nasライブラリの中からAE用のリソースがインストールされていることを確認
> AEのインストールロケーションに必要なファイル群をコピー
	インストール後はAEを再起動することを推奨
*/
...
if (typeof arguments == 'undefined') var arguments=new Array();
...
//	alert(nasFolderPath)
var thisFileName="nasOtomeSetup.jsx";//識別用自己ファイル名
var thisDataFileName="nas_Otome_setup.dat";//データファイル名識別用 同ディレクトリのみ
var thisPackageSign="//(nas_Otome_setup_data20160601)";//データの第一行目をすべて。これが一致しないと動作終了
...
//自前処理　localize がない場合(html等)は自前で処理
		...
		if(typeof myObject == "string"){
			...
			var myResult=myObject[myInstall.locale];
		}
