...
module nas.TCJ
タイムコードジャンプカウンタ	アクティブなコンポのタイムコード移動を行なう3連カウンタ
タイムシート1枚のフレーム数はプリファレンスパネルを使用してnasライブラリ共通変数で書き換えのこと
フレームレートは自動認識（コンポに従う）
アクティブなコンポは自動判別するが、「アクティブなコンポ」が必ずしも前面に出ていないことに注意すること
...
// ウインドウ初期化
	if((this)&&(this instanceof Panel)){
		...
		nas[moduleName][myWindowName]=nas.GUI.newWindow("palette","tcJumper",6.5,2,myLeft,myTop);
	}
if(! nas[moduleName].isDockingPanel){
...
//	オブジェクトプロパティ設定
	nas.TCJ.fct.targetComp=null;//まずnullで初期化する
		...
		nas.TCJ.fct.getTarget=function(){
			...
			if((this.targetComp)&&(nas.propCount(this.targetComp)<10)){this.targetComp=null}
...
nas.TCJ.fct.refresh=nas.GUI.addButton(nas.TCJ.fct,"★",0,myUIOffset+0.5,0.7,1);
	nas.TCJ.fct.refresh.onClick=function(){
		this.parent.getTarget();
		if(this.parent.targetComp){
			for(var c=0;c<3;c++)
			{
				this.parent.display[c].set(nas.ms2FCT(this.parent.targetComp.time*1000,this.parent.display[c].FCTtype,this.parent.display[c].FCTorg))
			}
		}
	}
//カウンタ作る
...
for(var n=0;n<3;n++)
{
	nas.TCJ.fct.display[n]=nas.GUI.addComboBox(nas.TCJ.fct,["00000","0:00:00","000 + 00","p 0 / 0 + 00","p 0 / + 000","00001 _","0:00:01 _","000 + 01 _","p 0 / 0 + 01 _","p 0 / + 001 _"],n,(2*n)+0.5,myUIOffset,2,1,true)
		nas.TCJ.fct.display[n].index=n;
		nas.TCJ.fct.display[n].FCTtype=[1,3,4][n];
		nas.TCJ.fct.display[n].FCTorg=[0,0,1][n];
	...
	nas.TCJ.fct.display[n].onChange=function(){
			...
			if(this.selected==null){
				...
				for(var c=0;c<3;c++){this.parent.display[c].set(nas.ms2FCT(this.parent.targetComp.time*1000,this.parent.display[c].FCTtype,this.parent.display[c].FCTorg))}
			}else{
