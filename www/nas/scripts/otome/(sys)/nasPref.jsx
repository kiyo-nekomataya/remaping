 ...
 *	直接	lib/config.js
 *		lib/nsa_OtomeLib.js を書き換えれば何でもかけますよ、と
 *
 */
//オブジェクト識別文字列生成 
var myFilename=("$RCSfile: nasPref.jsx,v $").split(":")[1].split(",")[0];
...
//ダイアログはパレットとオブジェクトの扱いがちがーう
	nas.Pref=nas.GUI.newWindow("dialog","各種設定",9,20,myLeft,myTop);
//コントロール定義
//メインコントロール

// TAB設定
	nas.GUI.setTabPanel(nas.Pref,["一般環境","メディア設定","動作設定","機能切替"],0,0,9,19);

//各タブにコントロールを配置
//	タブパネル０/一般環境設定
/*
config.js関連　乙女・りまぴん共通部分(nas)
*/
	nas.GUI.addStaticText(nas.Pref.tabPanel[0],"作業者名",0,1,2,1).justify="right";
...
nas.Pref.tabPanel[0].init=function()
{
	...
	this.stLength.text=nas.Frm2FCT(nas.SheetLength*nas.FRATE.rate,3);
	nas.workTitles.select();
...
//	タブパネル３　/　機能インストール
	...
	nas.GUI.addStaticText(nas.Pref.tabPanel[3],"このパネルの内容は記録されません。　切替はその場で反映されます",1,0,9,1).justify="right";

//乙女スタートアップ設定
	nas.GUI.addStaticText(nas.Pref.tabPanel[3],"スタートアップ",0,1,2,1).justify="right";
	nas.Pref.tabPanel[3].autoStartOn	=nas.GUI.addRadioButton(nas.Pref.tabPanel[3],"自動",3,1,2,1);
	nas.Pref.tabPanel[3].autoStartOff	=nas.GUI.addRadioButton(nas.Pref.tabPanel[3],"手動",5,1,2,1);
	nas.Pref.tabPanel[3].autoStartOn.value=(File("../../Startup/nasAeStartup.jsx").exists)?true:false;
	nas.Pref.tabPanel[3].autoStartOff.value=(File("../../Startup/nasAeStartup.jsx").exists)?false:true;

//乙女パネルユーティリティーインストール
	nas.GUI.addStaticText(nas.Pref.tabPanel[3],"パネル対応ツール",0,2,2,1).justify="right";
	nas.Pref.tabPanel[3].utInstall	=nas.GUI.addRadioButton(nas.Pref.tabPanel[3],"インストール",3,2,2,1);
	nas.Pref.tabPanel[3].utRemove	=nas.GUI.addRadioButton(nas.Pref.tabPanel[3],"アンインストール",5,2,2,1);
	nas.Pref.tabPanel[3].utInstall.value=(File("../../ScriptUI Panels/NasMenuII+.jsx").exists)?true:false;
	nas.Pref.tabPanel[3].utRemove.value=(File("../../ScriptUI Panels/NasMenuII+.jsx").exists)?false:true;
	
//フォルダ・クリップボード監視エージェント
	nas.GUI.addStaticText(nas.Pref.tabPanel[3],"フォルダ/クリップボード監視エージェント",0,4,4,1).justify="right";
	nas.Pref.tabPanel[3].saStop	=nas.GUI.addRadioButton(nas.Pref.tabPanel[3],"stop",3,5,2,1);
	nas.Pref.tabPanel[3].saWakeup	=nas.GUI.addRadioButton(nas.Pref.tabPanel[3],"wakeup",5,5,2,1);
	nas.Pref.tabPanel[3].saSleep	=nas.GUI.addRadioButton(nas.Pref.tabPanel[3],"sleep",7,5,2,1);
	nas.Pref.tabPanel[3].saStop.value=(nas.otome.systemWatcher.status=="stop")?true:false;
	nas.Pref.tabPanel[3].saWakeup.value=(nas.otome.systemWatcher.status=="wakeup")?true:false;
	nas.Pref.tabPanel[3].saSleep.value=(nas.otome.systemWatcher.status=="sleep")?true:false;

	nas.GUI.addStaticText(nas.Pref.tabPanel[3],"監視フォルダ",0,6,2,1).justify="right";
	nas.Pref.tabPanel[3].saChangeFolder=nas.GUI.addButton(nas.Pref.tabPanel[3],"変更",2,6,1,1);
	nas.Pref.tabPanel[3].saWatchFolder=nas.GUI.addEditText(nas.Pref.tabPanel[3],nas.otome.systemWatcher.watchFolder.fsName,3,6,6,1);

	nas.GUI.addStaticText(nas.Pref.tabPanel[3],"クリップボード監視",0,7,3,1).justify="right";
	nas.Pref.tabPanel[3].saWatchClip=nas.GUI.addSelectButton(nas.Pref.tabPanel[3],["enabled","disabled"],0,3,7,3,1);

//	コマンドキュー編集

//プリセットリスト
	nas.Pref.tabPanel[3].CQList=
			nas.GUI.addPanel(nas.Pref.tabPanel[3],"Queue編集",0.2,8,8.5,8.5);

//編集コントロール登録
	nas.Pref.tabPanel[3].CQList.init=function()
	{
	this.list=
		nas.GUI.addListBox(this,["no entry in commandQueue"],0,0.5,3,7,4);
	this.list.select(0);
	this.cqId=
		nas.GUI.addStaticText(this,"( "+"=id="+" )",0.3,1.6,0.7,1);
	this.cqId.justify="right";

	this.cqTarget=
		nas.GUI.addComboBox(this,["all","folder","clipboard","both","none"],4,1,1,3,1);
	this.cqFile=
		nas.GUI.addEditText(this,"=targetSig=",0.2,2.5,8,1);
	this.cqEnabled=
		nas.GUI.addEditText(this,"=on/off=",5,1.5,1,1);//on/off
...
nas.Pref.tabPanel[3].CQList.init();

//編集コントロール再登録
	nas.Pref.tabPanel[3].CQList.reInit=function()
	{
		var myList=new Array();
		for(var ix=0; ix<nas.otome.systemWatcher.commandQueue.length;ix++){
			myList.push(
				"[ "+ix+" ]"+
				nas.otome.systemWatcher.commandQueue[ix].watchTarget+" / "+ 
				nas.otome.systemWatcher.commandQueue[ix].watchFile.name+" / "+ 
				((nas.otome.systemWatcher.commandQueue[ix].enabled)?"enabled":"disabled" )
			)
		}
		this.list.setOptions(myList,0);
	
		if(! this.list.select()){this.list.select(0)};//未選択の場合だけ一番上のエントリを選択
		if(nas.otome.systemWatcher.commandQueue.length){
			this.cqId.text="( "+this.list.selected+" )";
			this.cqTarget.set(nas.otome.systemWatcher.commandQueue[this.list.selected].watchTarget);
			this.cqFile.text=nas.otome.systemWatcher.commandQueue[this.list.selected].watchFile.fsName;
			this.cqEnabled.text=nas.otome.systemWatcher.commandQueue[this.list.selected].enabled;
		}
	}
//画面アップデート
nas.Pref.tabPanel[3].CQList.update = function()
{
	if(nas.otome.systemWatcher.commandQueue.length){
		this.cqId.text	="( "+this.list.selected+" )";
		this.cqTarget.set(nas.otome.systemWatcher.commandQueue[this.list.selected].watchTarget);
		this.cqFile.text=nas.otome.systemWatcher.commandQueue[this.list.selected].watchFile.fsName;
		this.cqEnabled.text=nas.otome.systemWatcher.commandQueue[this.list.selected].enabled;
		this.delEntry.enabled=true;
	}else{
		this.cqId.text	="( -- )";
		this.cqTarget.set("--");
		this.cqFile.text="no entry";
		this.cqEnabled.text="--";
		this.delEntry.enabled=false;
	}
//	if(this.list.selected!=nas.inputMedias.selected){this.list.check(nas.inputMedias.selected)}
	this.chgEntry.enabled=false;
}

//nas.Pref.tabPanel[3].CQList.update();
//nas.Pref.tabPanel[1].oMediaList.update();
...
//Yes No Cancel
	nas.Pref.readButton=nas.GUI.addButton(nas.Pref,"読込",0,19,3,1);
	nas.Pref.writeButton=nas.GUI.addButton(nas.Pref,"保存",3,19,3,1);
...
*/
//tab[3]
//systemWatcher change
	chgSaWatcher=function(myStatus)
	{
		if(! myStatus){myStatus = this.text};
//alert(myStatus)
//	if(nas.otome.systemWatcher.status != myStatus)
	{nas.otome.systemWatcher(myStatus)};
	nas.Pref.SaWtUpdate();
	}

	nas.Pref.tabPanel[3].saStop.onClick	=chgSaWatcher;
	nas.Pref.tabPanel[3].saWakeup.onClick	=chgSaWatcher;
	nas.Pref.tabPanel[3].saSleep.onClick	=chgSaWatcher;

//表示のアップデート
nas.Pref.SaWtUpdate=function(myStatus){
			if(! myStatus){myStatus = nas.otome.systemWatcher.status;}
		switch (myStatus){
			case "stop": nas.Pref.tabPanel[3].saStop.value=true;break;
			case "wakeup": nas.Pref.tabPanel[3].saWakeup.value=true;break;
			case "sleep": nas.Pref.tabPanel[3].saSleep.value=true;break;
		}
}
//remove QueueCommand
	nas.Pref.tabPanel[3].CQList.delEntry.onClick=function()
	{
		nas.otome.systemWatcher.removeCommand(this.parent.list.selected);
		this.parent.reInit();
		this.parent.update();
	};

//AutoStartChange
	function chgStartup(status)
	{
		var myStartupTargetPath		=[Folder.scripts.path,"Scripts/Startup/nasAeStartup.jsx"].join("/");
		var myShutdownTargetPath	=[Folder.scripts.path,"Scripts/Shutdown/nasAeShutdown.jsx"].join("/");
		var myStartupScriptPath		=[Folder.nas.path,"nas/scripts/otome/nasAeStartup.jsx"].join("/");
		var myShutdownScriptPath	=[Folder.nas.path,"nas/scripts/otome/nasAeShutdown.jsx"].join("/");
		if(status=="auto")
		{
			if(! File(myStartupTargetPath).exists)
			{File(myStartupScriptPath).copy(myStartupTargetPath);	};
			if(! File(myShutdownTargetPath).exists)
			{File(myShutdownScriptPath).copy(myShutdownTargetPath);	};
		}else{
			if(File(myStartupTargetPath).exists)
			{File(myStartupTargetPath).remove();	};
			if(File(myShutdownTargetPath).exists)
			{File(myShutdownTargetPath).remove();	};				
		}
	}	
	nas.Pref.tabPanel[3].autoStartOn.onClick	=function(){chgStartup("auto");};
	nas.Pref.tabPanel[3].autoStartOff.onClick	=function(){chgStartup("manual");};

//PanelToolsSetup
	function setPanels(status)
	{
		myTargets=[
		[Folder.scripts.path,"Scripts/ScriptUI Panels/nasTCJump2.jsx"].join("/"),
		[Folder.scripts.path,"Scripts/ScriptUI Panels/dbgConsole.jsx"].join("/"),
		[Folder.scripts.path,"Scripts/ScriptUI Panels/NasOtome.jsx"].join("/"),
		[Folder.scripts.path,"Scripts/ScriptUI Panels/NasMenuII+.jsx"].join("/")
		];
		myTools=[
		[Folder.nas.path,"nas/scripts/otome/nasTCJump2.jsx"].join("/"),
		[Folder.nas.path,"nas/scripts/otome/dbgConsole.jsx"].join("/"),
		[Folder.nas.path,"nas/scripts/otome/NasOtome.jsx"].join("/"),
		[Folder.nas.path,"nas/scripts/otome/NasMenuII+.jsx"].join("/")
		];
			var counter=0;
			var names=new Array();
		if(status=="install")
		{
			...
			for(ix in myTargets){
				if(! File(myTargets[ix]).exists)
				{
					...
					var msg=names.join(" ,")+nas.GUI.LineFeed+counter +" 個の　パネルユーティリティーをインストールしました。AEを再起動後に有効になります。";
					alert(msg);
		}else{
			for(ix in myTargets){
				if(File(myTargets[ix]).exists)
				{
					File(myTargets[ix]).remove(); counter++;
					counter++;
					names.push(File(myTargets[ix]).name);
				};
			}
					var msg=names.join(" ,")+nas.GUI.LineFeed+counter +" 個の　パネルユーティリティーを削除しました。";
					alert(msg);
		}
	}	
	nas.Pref.tabPanel[3].utInstall.onClick	=function(){setPanels("install");};
	nas.Pref.tabPanel[3].utRemove.onClick	=function(){setPanels("remove");};
//Yes/No/Cacel
