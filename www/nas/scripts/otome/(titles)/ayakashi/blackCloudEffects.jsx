 ...
 */
//オブジェクト識別文字列生成 
var myFilename="blackCloudEffects.jsx";//正式なファイル名と置き換えてください。
var myFilerevision="0.01";//ファイルージョンはアバウトパネルで表示されます。

/*	cvs/rcs使用の方は、下の2行をご使用ください
	var myFilename=("$RCSfile$").split(":")[1].split(",")[0];
	var myFilerevision=("$Revision$").split(":")[1].split("$")[0];
//
	*/
var exFlag=true;
var moduleName="blackCloud";//識別用のモジュール名で置き換えてください。
//二重初期化防止トラップ
try{
	if(nas.Version)
	{
...
//ターゲットのコンポにすでに「XXX効果」レイヤがある場合は処理スキップ
	if(app.project.activeItem.layers.byName("黒雲効果(下地)")){alert("コンポに「黒雲効果」レイヤがあります。\nすでに処理済みのようなので処理をスキップします。");
	}else{
...
//ターゲットレイヤが複数の場合は、プリコンポーズしてターゲットレイヤをひとつにする。
	if(app.project.activeItem.selectedLayers.length==1){
		...
		var newCompName=app.project.activeItem.name+"黒雲プリコンポ";
...
//事前処理プリコンポ作成
		var newCompName=app.project.activeItem.name+"黒雲事前処理";
...
//	マチエル設定
		textureLayer=myPrecomp.layers.addSolid([1,1,1],"黒雲マチエル",myPrecomp.width,myPrecomp.height,myPrecomp.pixelAspect,myPrecomp.duration);
...
//	調整レイヤを作成　カーブ設定　モード変更覆い焼き50%
	myAdjustmentLayer=targetComp.layers.addSolid([1,1,1],"色調整",targetComp.width,targetComp.height,targetComp.pixelAspect,targetComp.duration);
			...
			myAdjustmentLayer.name="黒雲効果(色付け)";
//	ターゲットレイヤをモード変更スクリーン50%
	targetLayer.blendingMode=BlendingMode.SCREEN;
	targetLayer.opacity.setValue(25);
	targetLayer.name="黒雲効果(下地)";
//	
