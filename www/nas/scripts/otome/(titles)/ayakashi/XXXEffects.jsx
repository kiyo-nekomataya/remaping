 ...
 */
//オブジェクト識別文字列生成 
//var myFilename="-XXX-.jsx";//正式なファイル名と置き換えてください。
...
var exFlag=true;
var moduleName="-XXX-";//識別用のモジュール名で置き換えてください。
//二重初期化防止トラップ
try{
	if(nas.Version)
	{
...
//常駐部分の初期化
if(exFlag){
	/*----この下に常駐部分のスクリプトを記述してください。----*/
//	各種プロパティ・メソッド等を初期化


};

	/*---- この下には非常駐部分のコマンドを置きます。main相当 ----*/
//サンプルです。アクティブコンポの選択レイヤを複製します。複数選択の場合はプリコンポして複製。
//(*注 実用価値はないです。処理参考)
			if(!( app.project)){
//	プロジェクトがないのでもう何もしません。
			}else{
...
//ターゲットのコンポにすでに「XXX」レイヤがある場合は処理スキップ
	if(app.project.activeItem.layers.byName("XXX")){alert("コンポに「XXX」レイヤがあります。\nすでに処理済みのようなので処理をスキップします。");
	}else{
...
//	undoブロック開始
	...
	var dupLayer=targetLayer.duplicate();
	dupLayer.name="XXX";
