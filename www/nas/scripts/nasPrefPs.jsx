...
alert("nas-library is required./nasライブラリが必要です\nPlease startup or load nas-library./Photoshop_Startup.jsx を実行してください");
	exFlag=false;
}
...
//	現在の解像度とフレームレートをバックアップ
    ...
    var currentFramerate  = nas.FRATE.rate;
//		サブプロシジャ
...
//編集コントロール登録
	...
	this.list=
		nas.GUI.addListBox(
			this,
			nas.workTitles.names("all"),
			nas.workTitles.selected,
			0.5,3.5,7,4
		);

/*		nas.GUI.addDropDownList(
			w.imPanel,
			nas.inputMedias.names(0),
...
//IM
	this.linkIM=
		nas.GUI.addSelectButton(
			this,
			nas.inputMedias.names(),
			nas.workTitles.selectedRecord[3],
		1,2.5,3,1
	);// */
/*	this.linkIM = nas.GUI.addDropDownList(
		this,
		nas.inputMedias.names(),
		nas.workTitles.selectedRecord[3],
		1,2.5,3,1
...
{
	this.Uname.text=(nas.CURRENTUSER instanceof nas.UserInfo)?nas.CURRENTUSER.handle:nas.CURRENTUSER;
...
nas.Pref.tabPanel[1].MediaList.update = function()
{
	...
	nas.Pref.tabPanel[0].fRate.text=nas.FRATE.name;
}
...
//Tab[0]
//nas_common
	nas.Pref.tabPanel[0].Uname.onChange=function(){nas.CURRENTUSER.setValue(this.text)};
	nas.Pref.tabPanel[0].bResolution.onChange=function(){nas.RESOLUTION=new nas.UnitResolution(this.text,'dpi');this.text=nas.RESOLUTION.as('ppi')};
	nas.Pref.tabPanel[0].fRate.onChange=function(){if(! isNaN(this.text))nas.FRATE.setValue(this.text);this.text=nas.FRATE.name;};
	nas.Pref.tabPanel[0].stLength.onChange=function(){var frames=nas.FCT2Frm(this.text);if(! isNaN(frames)){nas.SheetLength=frames/nas.FRATE.rate};this.text=nas.Frm2FCT(frames,3);};
...
//Yes/No/Cacel
	nas.Pref.importButton.onClick=function(){nas.importPreference();this.parent.init("all");};
	nas.Pref.exportButton.onClick=function(){nas.exportPreference();};
	...
	ja:"保存中の設定を読み込みます。現在の設定は上書きされます。\n取り消しはできません。よろしいですか？"
		...
		if(doAction){nas.readPreference();this.parent.init("all")};
		};
	...
	ja:"設定を[ %1 ]以下に保存します。\n以前のファイルは上書きされます。よろしいですか？"
		...
		if(doAction){nas.writePreference();}
		};
	nas.Pref.clearButton.onClick=function(){nas.cleraPreference();};
...
//Start GUI
    nas.RESOLUTION.setValue(currentResolution,'dpi');
    nas.FRATE.setValue(currentFramerate);
	nas.Pref.init("0");
nas.Pref.show();
//
}
