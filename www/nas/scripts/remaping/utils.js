...
//console.log([insertPoint[0],new Xps.XpsTimelineTrack(trackName,myOpt,newXPS.xpsTracks,currentDuration)]);
...
interpSign=function(chr){
  ...
  if(xUI.Selection.join(",")=="0,0"){
	if(myValue.match(interpRegex)){
		...
		xUI.sheetPut(nas_expdList(nas.CellDescription.interpolationSigns[0]));
		xUI.spin("down");
	}else{
		return;
	}
