...
var mySplash=basePath+"/nas/images/psAXEsplash.png";
var myInstall=basePath+"/nas/setup/psAxe/psAxeInstall/nasPsAxeInstall.jsx";
//CSX (CS6 or CC)を検出時はパスを更新
if(File(basePath).name=="HTML"){
  basePath=File(basePath).parent.fullName;
	mySplash=basePath+"/nas/psAXEsplash.png";
	myInstall=basePath+"/HTML/psAxeInstall/nasPsAxeInstall.jsx";
}
//alert([basePath,mySplash,myInstall].join('\n'))

if( typeof nas =="undefined"){nas=new Object;nas.Version=" --------- ";app.nas=nas;}
...
myText02=w.add("staticText",[64,490,448,512],"photoshop animation tools by nekomataya / ver "+app.nas.Version.psAxe+" / kiyo 2021");
//インストールロケーション取得
...
if(File(myInstallFolder).exists){
    myButton02=w.add("button",[128,414,384,438],"Install/Uninstall");
    myButton03=w.add("button",[128,450,384,474],"new FrameTemplate");
    w.remove(myText01);
    myButton01.text="OK";
    ...
    myButton03.onClick=function(){
        if(File(myInstallFolder+"/scripts/mkFrame.jsx").exists)
        {
            $.evalFile(myInstallFolder+"/scripts/mkFrame.jsx");
        }
    };
}
w.show();
