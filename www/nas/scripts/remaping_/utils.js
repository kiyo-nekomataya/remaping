...
console.log([insertPoint[0],new nas.Xps.XpsTimelineTrack(trackName,myOpt,newXPS.xpsTracks,currentDuration)]);
	newXPS.insertTL(insertPoint[0],new nas.Xps.XpsTimelineTrack(trackName,myOpt,newXPS.xpsTracks,currentDuration));
//	xUI.selection();
//	xUI.selectCell('0_0')
...
//タイムラインラベル変更用ボタンメソッド
var inputButtonText = function(myText){
...
interpSign=function(chr){
	...
	if(xUI.Selection.join(",")=="0,0"){
		if(myValue.match(interpRegex)){
			...
			xUI.sheetPut(newValue);
		}else if(true){
			xUI.sheetPut((chr)?nas_expdList(chr):nas_expdList(nas.CellDescription.interpolationSigns[0]));
			xUI.spin("down");
		}else{
			return;
		}
		...
		xUI.sheetPut(nas_expdList((chr)?"/"+chr+"/":"/-/"));
//		xUI.sheetPut(nas_expdList((chr)?"/"+chr+"/":"/○/"));
...
//編集可能なケースでは、undoシステムに組み込む
		xUI.sheetPut(mainXps,[0,0],false,function(){
			xUI.selectCell(bkPos);	//バックアップ位置へ復帰
		})
	};
 ...
 *		配列渡し
 *	@params	{Object|nas.Xps.XpsTimelineTrack} targetTrack
...
var iptFilter = function(cell,targetTrack,mode,exch){
    ...
    if(!(targetTrack instanceof nas.Xps.XpsTimelineTrack)) return cell;
	if(! mode) mode = 0;
	if(! exch) exch = false;
	var excStack = [];
...
//選択解除＞開始フレームへ移動
	...
	xUI.sheetPut(nas_expdList(label+'"'+content+'"'));
}
/*TEST
