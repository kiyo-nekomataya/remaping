...
var currentName=nas.workTitles.bodys[nas.axe.dmCurrent[0]][2]+'#'+nas.RZf(nas.axe.dmCurrent[1],2)+ "__" +nas.RZf(nas.axe.dmCurrent[2],3);
...
// w.fileName= nas.GUI.addEditText(w,nas.incrStr(currentName),2,0,5,1);
 ...
 w.lb3 = nas.GUI.addStaticText(w,"( TIME )",4.5,1,1.3,1);//
...
w.edPanel.selectCl=nas.GUI.addDropDownList(w.edPanel,nas.axe.lyBgColors,2,3,2,3,1);
...
//name formatをUAT互換に変更 2020.0825
...
//=======================================作成
 ...
 //A3縦位置固定　解像度は特に変更が無ければ200dpi
  ...
  var myName=w.fileName.text+"_xps";
...
if(myLayerCounts>0){
//シート罫線を読み込み
  var myRuleFile=new File(Folder.nas.fullName+"/lib/resource/timeSheet6sA3.svg");
...
var myTimeLayer=myWorkSets[0].artLayers.add();//レイヤ追加
  ...
  myTimeLayer.translate(
	new UnitValue(((myRuleLayer.bounds[0]-myTimeLayer.bounds[0]).as("mm")+190)+" mm"),
...
//ここでペースト
if(clipB){
    var pasteSheetLayer=myWorkSets[0].artLayers.add ();
    pasteSheetLayer.name="sheet";
	app.activeDocument.activeLayer=pasteSheetLayer;
