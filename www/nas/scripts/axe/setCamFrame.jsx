...
//+++++++++++++++++++++++++++++++++ここまで共用
//現在のアクティブレイヤーの直上にカメラフレームを読み込み

var myTargetSet   = false;
...
//100フレーム枠を読み込み
  var myFrameFile=new File(Folder.nas.fullName+"/lib/resource/Frames_cam/"
...
//フレーム配置　今日はセンタリングのみで左右はパス 20110820
  ...
  if(myPegLayer) myOffset -= (myPegLayer.bounds[3]/2).as("mm");//タップからの距離を減算
  myFrameLayer.name="frame";//この操作が取り消し対象ダミー
  myFrameLayer.translate(new UnitValue("0 mm"),new UnitValue((-myOffset)+" mm"));
...
//alert(Folder.nas.fullName+"/lib/resorce/FieldCharts/fieldChart"+myWidth.toString()+"F.svg");
 var myFieldChart=new File(Folder.nas.fullName+"/lib/resource/FieldCharts/fieldChart"+myWidth.toString()+"F.svg");
//セット
...
//if(myTargetSet){
//    myFrameLayer.move(currentActiveLayer,ElementPlacement.PLACEBEFORE);
...
//}
//var names = [];
//for (ver i = 0; i < myFrameLayer.parent.layers.length;i++) names.push(myFrameLayer.parent.layers[i].name);
//names.filter(function(e){e.match(/^\[([0-9A-Z])\]$/)})
//  if(!bootFlag){
//    myPegLayer.name="peg";
    myFrameLayer.name="[A]";
...
if (myTarget.mode==DocumentMode.BITMAP){
	var myFile=new File(Folder.nas.fullName+"/lib/resource/Pegs/peg3p.svg");
