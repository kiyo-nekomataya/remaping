...
CS6でアニメーションが初期化されていない状態が発生したのでその対応（エラー停止の回避）コードを暫定的に追加
20120617

マルチステージ対応
マルチステージが設定されている場合はステージをセレクトする操作が必用
ステージは、任意のステージを選択可能で出力指定されたステージはステージ名のディレクトリを作成してその配下に出力する
従来の操作も同時に行うので任意の単一ステージのみを出力する場合はルートフォルダに出力ステージとシステムフォルダ以外を配置しないようにドキュメント自体を配置する必要がある

従来の出力配置を（名前なし）ルートステージと呼称する
ステージ名はブラケットで囲むか、または数字で始まるものとする
ステージと認識されたレイヤーセットは検索深度が１段階深くなる
[LO]
02_LO

放置されていた ルートフォルダのファイルが一時ファイルの名前のフォルダに保存されるバグを修正 20201222
...
//操作用の一時ドキュメントを作っておく
 ...
 exportFiles.stageList=[];
/*
	引数オブジェクトがメンバ条件を満たすか否かを判定
	アートレイヤーでノーマルまたはスマートオブジェクト
	レイヤーセットで、描画モードがノーマルかつ レイヤーセット配下に何らかのレイヤーを保持しているもの
*/
exportFiles.isMember=function(myObject){
	if(
		( myObject.typename=="ArtLayer")&&
		((myObject.kind==LayerKind.NORMAL)||(myObject.kind==LayerKind.SMARTOBJECT))
	) return true;
	if(
		(myObject.typename=="LayerSet")&&
		(myObject.layers.length >= 0)&&
		(myObject.blendMode == BlendMode.NORMAL)
	) return true;
	return false;
}
exportFiles.refresh=function(stageIndex){
//	if(! stageIndex) return false;
//リセット時の再取得も含むので配列を初期化
	this.guideLayers.length=0;this.outputList.length=0;
	this.guideLayersA.length=0;this.outputListA.length=0;
	this.stageList.length=0;
	for(var ix=0;ix<this.tempDoc.layers.length;ix++){
		var currentIdf=this.tempDoc.layers[ix].name;//レイヤー名を取得

/* 第一階層のレイヤーを検査
    フレーム・ペグ等の補助レイヤーの名前がついているものを取得して控える
    テンポラリードキュメント及びオリジナルドキュメントの両方
*/
    
		if(currentIdf.match(/(frames?|peg|memo|note|system)|(^_.*)/i)){
			this.guideLayers.push(this.tempDoc.layers[ix]);
			this.guideLayersA.push(this.targetDoc.layers[ix]);
			continue;
		};
/*
    レイヤーセットの名称が[ブラケット]付き|数字開始 の場合はステージフォルダとして控える
    ステージフラグを立てる
*/
		if((this.tempDoc.layers[ix].typename == "LayerSet")&&(currentIdf.match(/^\[[^\[]+\]$|^[0-9]/))){
			this.stageList.push(this.tempDoc.layers[ix]);
			var stg = this.tempDoc.layers[ix];
//ステージ内処理
			if(stg.layers.length > 0)
			for(var lx = 0 ; lx < stg.layers.length ; lx ++){
				if((stg.layers[lx].typename != 'LayerSet')&&(this.isMember(stg.layers[lx]))){
//stg第一階層のノーマルレイヤーを処理
					this.outputList.push(stg.layers[lx]);
					this.outputListA.push(this.targetDoc.layers[ix].layers[lx]);
				}else if(
					(stg.layers[lx].typename == "LayerSet")&&
					(stg.layers[lx].layers.length > 0)
				){
//第二階層はレイヤーセットも処理
					for(var mx = 0 ; mx < stg.layers[lx].layers.length ; mx ++){
						if(this.isMember(stg.layers[lx].layers[mx])){
							this.outputList.push(stg.layers[lx].layers[mx]);
							this.outputListA.push(this.targetDoc.layers[ix].layers[lx].layers[mx]);
						};
					};
				};
			};
...
//================ArtLayerのうちノーマルレイヤーとスマートオブジェクトを選択
		if(
			(this.tempDoc.layers[ix].typename != "LayerSet")&&
			(this.isMember(this.tempDoc.layers[ix]))
		){
			this.outputList.push(this.tempDoc.layers[ix]);
			this.outputListA.push(this.targetDoc.layers[ix]);
			continue;
		}else if(
			(this.tempDoc.layers[ix].typename == "LayerSet")&&
			(this.tempDoc.layers[ix].layers.length)
		){
//=================レイヤセットでかつ、配下にレイヤを含んでいる場合（１段のみ掘下げ レイヤセットもメンバーにする）
			for (var lx=0;lx<this.tempDoc.layers[ix].layers.length;lx++){
				if(this.isMember(this.tempDoc.layers[ix].layers[lx])){
...
//=============プレフィクスは、ターゲットから取得する
		...
		for(var fl = 0 ; fl < this.outputList.length ; fl ++){
			if((! this.mkFolder)||(this.outputList[fl].parent.typename == "Document")){
				this.outputListView.push(this.outputList[fl].name.replace(/[\\\/\:\?\*\"\>\<\|]/g,"_"));
			}else{
				var folders = [this.outputList[fl].parent.name.replace(/[\\\/\:\?\*\"\>\<\|]/g,"_")];
				if((this.outputList[fl].parent.parent)&&(this.stageList.indexOf(this.outputList[fl].parent.parent) >= 0 )){
					folders.push(this.outputList[fl].parent.parent.name.replace(/[\\\/\:\?\*\"\>\<\|]/g,"_"));
				};
				this.outputListView.push("[ "+ folders.reverse().join("/") +" ] "+this.outputList[fl].name.replace(/[\\\/\:\?\*\"\>\<\|]/g,"_"));
			}
		};
	};
}
    ...
    for(var fidx=0;fidx<this.outputList.length;fidx++){
       //レイヤをたどって指定に従ってガイドレイヤのあるなしを選択して出力
                if(this.mkFolder){
                    ...
                    var stageName  = false;
                    if(
                    	(this.outputList[fidx].parent.typename == 'LayerSet')&&
                    	(this.stageList.indexOf(this.outputList[fidx].parent.parent) >= 0)
                    ){
                    	stageName = this.outputList[fidx].parent.parent.name.replace(/[\\\/\:\?\*\"\>\<\|]/g,"_");
                    }

                    if(folderName == '__exportTempDoc__'){
                        myTargetFolder = this.currentTargetFolder;
                    }else if(stageName){
                        myTargetFolder = new Folder(this.currentTargetFolder.fullName+"/"+stageName+"/"+folderName);
                    }else{
...
psdドキュメントに存在しないレイヤを指すフレームやアニメサムネールのキャッシュがつくので何かと不都合
アニメフレームを削除するために初期化を行う。
単純な初期化では第一フレームの表示状態が再生されるため、当該レイヤが非表示のまま保存される場合があるので再表示を行う。
*/
             app.activeDocument=mySaveDocument;nas.axeAFC.initFrames();//アニメフレームを初期化
             mySaveDocument.layers[0].visible=true;//表示が消えるので復帰しておく
...
//for(var ecItem in this.parent.fileList.selection)
for(var ecItem = 0 ; ecItem < this.parent.fileList.selection.length ;ecItem ++){removeItemList.push(this.parent.fileList.selection[ecItem].index);}
