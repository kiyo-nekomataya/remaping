...
/** 第一階層のレイヤーを検査
    ...
    レイヤーセットの名称が[ブラケット]付きの場合はステージをあらわす。
*/
    
	if(currentIdf.match(/(frames?|peg|memo|system)|(^_.*)/i)){
	 this.guideLayers.push(this.tempDoc.layers[ix]);
	 this.guideLayersA.push(this.targetDoc.layers[ix]);
	 continue;
	}else if(currentIdf.match(/^\[[^\[]+\]$/)){
...
//================ArtLayerのうちノーマルレイヤーとスマートオブジェクトを選択
	if(this.isMember(this.tempDoc.layers[ix])){this.outputList.push(this.tempDoc.layers[ix]);this.outputListA.push(this.targetDoc.layers[ix]);continue;}
