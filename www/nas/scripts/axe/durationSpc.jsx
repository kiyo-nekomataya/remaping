...
_aplDur=function(){
	...
	if(! isNaN(myFrames)){
	   if(animationMode=="timelineAnimation"){nas.axeVTC.setDuration(myFrames);}else{nas.axeAFC.setDly(myFrames/nas.FRATE.rate)};
		this.parent.close();
	}
}
...
w.cb23=nas.GUI.addComboBox(w,[15,24,23.98,25,30,29.97,59.94,60,100],nas.FRATE.rate,3,.5+vOffset,2,1)
...
//======================================================================
w.sl00.onChange=function(){w.cb21.set(nas.Frm2FCT(Math.floor(this.value*nas.FRATE.rate),3,0));w.cb21.onChange();}

Slider.on
w.cb21.onChange=function(){this.parent.setFrames=nas.FCT2Frm(this.value)}
w.cb23.onChange=function(){nas.FRATE.rate=nas.newFramerate("",parseFloat(this.value));if(animationMode=="timelineAnimation"){nas.axeVTC.setFrameRate();}};
