...
var currentName=[
    nas.workTitles.bodys[nas.axe.dmCurrent[0]][2],
    "#",
    nas.RZf(nas.axe.dmCurrent[1],2),
    "__",
    nas.RZf(nas.axe.dmCurrent[2],3)
    ].join('');
...
//ダイアログを出力してドキュメントの指定条件を取得
 ...
 w.lb0 = nas.GUI.addStaticText(w,nas.localize(nas.uiMsg.fileName),0,0,2,1);//"ファイル名"
// w.fileName= nas.GUI.addEditText(w,nas.incrStr(currentName),2,0,5,1);
 ...
 w.lb3 = nas.GUI.addStaticText(w,"( TIME )" ,4.7,1,1.3,1);

	w.opusNumber= nas.GUI.addEditText(w,nas.RZf(nas.axe.dmCurrent[1],2),0.5,2,1,1);
...
// w.cutNumber= nas.GUI.addEditText(w,nas.incrStr(nas.axe.dmCurrent[2].toString()),3,1,1,1);
	w.cutNumber= nas.GUI.addEditText(w,nas.axe.dmCurrent[2].toString(),2.5,2,1.5,1);
	  w.cutInc= nas.GUI.addButton(w,"+" ,3.8   ,2,0.75,1);
	  w.cutDec= nas.GUI.addButton(w,"-" ,4.25 ,2,0.75,1);
...
　w.imPanel=nas.GUI.addPanel(w,nas.localize(nas.uiMsg.Frame),0,3,7,6);//"フレーム"

w.imPanel.lb0 = nas.GUI.addStaticText(w.imPanel,nas.localize(nas.uiMsg.title)+":",0,0.5,2,1);//"タイトル:"
w.imPanel.selectTT=nas.GUI.addComboBox(w.imPanel,nas.workTitles.names(0),nas.workTitles.selected,2,0.5,4,1)

w.imPanel.lb1 = nas.GUI.addCheckBox(w.imPanel,":"+nas.localize(nas.uiMsg.Frame),0,2,3,1);//":フレーム"
...
w.imPanel.lb3 = nas.GUI.addCheckBox(w.imPanel,":"+nas.localize(nas.uiMsg.peg),0,3,3,1);//":タップ"
...
w.cutNumber.onChange=function(){
    var sci = nas.Pm.parseCutIF(this.text);
    this.text = "s"+((sci.length > 1)? sci[1]:'')+"-c"+sci[0];
    this.parent.fileName.update();
};
...
var myCutLayer=myFrameSet.artLayers.add();//レイヤ追加
  ...
  myCutLayer.translate(
	new UnitValue((myTextLayer.bounds[2].as("mm")+10)+" mm" ),
	new UnitValue(((myPegLayer.bounds[3]-myCutLayer.bounds[1]).as("mm")+5)+" mm" )
  );//制作番号のバウンディングボックスを基準に配置を調整
