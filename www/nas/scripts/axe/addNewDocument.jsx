/* addNewDocument.jsx
    ...
    フレームを透過50%に 20191115
    選択範囲の複製を背景レイヤー検出仕様に変更 20201218
    文字配置を更新してセンターホールの真下を開ける 20201229
    ライブラリローダの判定を追加 20220822
*/
...
// 選択範囲の存在を確認する バウンズ取得を試みて失敗すればなし
function checkSelection(){
var flg = true;try {flg = (activeDocument.selection.bounds)? true:false;}catch(e){flg = false;};return flg;}
//ドキュメント情報からドキュメント名を作成する
var currentName = "";
if(nas.workTitles.bodys[nas.axe.dmCurrent[0]]) currentName = nas.workTitles.bodys[nas.axe.dmCurrent[0]][2]+'#'+nas.RZf(nas.axe.dmCurrent[1],2)+ "__" +nas.RZf(nas.axe.dmCurrent[2],3);
...
var openPhotoshopDialog = function(oldWindow){
	if((oldWindow)&&(oldWindow.close)) 	oldWindow.close();
	var myExcute="";
//=============== コード
// =======================================================新規ドキュメント作成
...
//alert([nas.registerMarks.names(0),nas.registerMarks.selected]);
if(nas.axe.dmDialog){
...
// w.fileName= nas.GUI.addEditText(w,nas.incrStr(currentName),2,0,5,1);
 ...
 w.lb3 = nas.GUI.addStaticText(w,"( TIME )",4.2,1,1.3,1);//
	...
	w.opusNumber= nas.GUI.addEditText(w,nas.RZf(nas.axe.dmCurrent[1],2),1,1,1.5,1);
...
// w.cutNumber= nas.GUI.addEditText(w,nas.incrStr(nas.axe.dmCurrent[2].toString()),3,1,1,1);
	w.cutNumber= nas.GUI.addEditText(w,nas.axe.dmCurrent[2].toString(),3,1,1.5,1);
...
/*
    ...
    タップとフレームの表示初期値
 */
w.headlineTxt=nas.GUI.addCheckBox(w,nas.localize({en:"add headline text",ja:"テキスト表示"}),7,7,2,2);
    w.headlineTxt.value = true;
w.headlineTxt.onClick=function(){
	w.fileName.update();
};
...
//サイズを8ピクセル単位で近接の値に切り上げる
w.imPanel.etWIDTH = nas.GUI.addEditText(w.imPanel,Math.ceil(nas.decodeUnit(nas.paperSizes.selectedRecord[1]+"mm","px")/8)*8,3,6,2,1);
w.imPanel.etHEIGHT = nas.GUI.addEditText(w.imPanel,Math.ceil(nas.decodeUnit(nas.paperSizes.selectedRecord[2]+"mm","px")/8)*8,3,7,2,1);
...
//=========================
 ...
 w.psBt=nas.GUI.addButton(w,"psDialog",7,3,2,1);
/*
...
w.imPanel.selectTT.onChange=function(){
	if(this.selected){
		nas.workTitles.select(this.selected);//既存の選択タイトルを切替
		nas.axe.dmCurrent[0]=nas.workTitles.selected;
	}else{
//既存タイトルでない場合はworkTitlesに登録するかどうかを選択したほうが良い		
		var newRecord = [
			w.imPanel.selectTT.value,
			w.imPanel.selectTT.value,
			w.imPanel.selectTT.value,
			nas.workTitles.selectedRecord[3],
			nas.workTitles.selectedRecord[4]
		];
		nas.workTitles.select(nas.workTitles.bodys.add(newRecord));
		if(startup){nas.writePreference();}
	};
...
w.imPanel.selectIM.onChange=function(){
 ...
 nas.RESOLUTION.setValue(nas.inputMedias.selectedRecord[3],"dpi");//dpc
...
w.fileName.update=function(){
	...
	if(! w.headlineTxt.value) myPrefix = "";
var currentName = myPrefix+'#'+
	this.parent.opusNumber.text+
	"__" +this.parent.cutNumber.text;
this.text=currentName;
};
//値上下ボタン
w.opusInc.onClick=function(){this.parent.opusNumber.text=nas.RZf(nas.incrStr(this.parent.opusNumber.text),2);this.parent.opusNumber.onChange();};
w.opusDec.onClick=function(){this.parent.opusNumber.text=nas.RZf(nas.incrStr(this.parent.opusNumber.text,-1),2);this.parent.opusNumber.onChange();};
w.cutInc.onClick=function(){this.parent.cutNumber.text=nas.RZf(nas.incrStr(this.parent.cutNumber.text),3);this.parent.cutNumber.onChange();};
w.cutDec.onClick=function(){this.parent.cutNumber.text=nas.RZf(nas.incrStr(this.parent.cutNumber.text,-1),3);this.parent.cutNumber.onChange();};

w.secInc.onClick=function(){this.parent.timeText.text=nas.Frm2FCT(nas.FCT2Frm(this.parent.timeText.text)+nas.FRATE.rate,3);};
w.secDec.onClick=function(){this.parent.timeText.text=nas.Frm2FCT(nas.FCT2Frm(this.parent.timeText.text)-nas.FRATE.rate,3);};
...
//解像度が直接変更された場合、nas.RESOLUTIONを更新
 w.imPanel.etRESOLUTION.onChange=function(){
	if(isNaN(this.text)){
		...
		nas.RESOLUTION.setValue(this.text,'dpi');
	};
 };
//=======================================作成
...
//レジスタ(peg)
   ...
   myPegLayer.move(myWorkSets[0],ElementPlacement.PLACEATEND);
	myPegLayer.translate(
		new UnitValue("0 mm"),
		new UnitValue(-1*myPegLayer.bounds[1].as('mm')+' mm'));//上辺へはっつけ
//100Fldフレーム枠を読み込み
   var myFrameFile=new File(Folder.nas.fullName+"/lib/resource/Frames/"
	+Math.floor(nas.inputMedias.selectedRecord[1])+"mm"
	+nas.inputMedias.selectedRecord[2].replace(/\//,"x")
	+".svg"
   );
   var myFrameLayer=nas.axeAFC.placeEps(myFrameFile);
//フレーム配置　今日はセンタリングのみで左右はパス 20110820
   ...
   myFrameLayer.translate(
		new UnitValue("0 mm"),
		new UnitValue(((myPegLayer.bounds[3]/2).as("mm")-myOffset)+" mm"));//タップからの距離を */
   if(nas.axe.frameOpc){myFrameLayer.opacity=50;};
   myFrameLayer.name="frame";
   if(w.headlineTxt.value){
//各種テキストを配置
    ...
    myCutLayer.textItem.contents = this.parent.cutNumber.text;
/*
    myCutLayer.translate(
	new UnitValue((myTextLayer.bounds[2].as("mm")-myCutLayer.bounds[0].as("mm")+10)+" mm"),
	new UnitValue(((myTextLayer.bounds[1]-myCutLayer.bounds[1]).as("mm"))+" mm")
    );//タップ位置を基準に調整 */
	myCutLayer.translate(
		new UnitValue(120+" mm"),
		new UnitValue(((myPegLayer.bounds[3]/2).as("mm"))+" mm")
	);//タップからの距離を
...
if(bootFlag){nas.writePreference(["nas.axe","nas.RESOLUTION","nas.workTitles.selected","nas.registerMarks.selected"]);}
//alert(nas.axe.dmCurrent)
 	this.parent.close();
};
//キャンセル
 w.cnBt.onClick=function(){this.parent.close();};
//photoshopのダイアログを呼び出して終了
 w.psBt.onClick=function(){
	openPhotoshopDialog(this.parent);
 };
/*
