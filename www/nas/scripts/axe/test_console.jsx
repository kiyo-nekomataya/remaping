...
{
if (! myText.length){alert(nas.localize({en:"no data to save.",ja:"保存するデータがありません"}));return false;}
if(isWindows)
{
	var mySavefile = File.saveDialog(nas.localize({en:"specify the file name for export",ja:"書き出しのファイル名を指定してください"}),"File (*.js *.jsx *.txt):*.JS;*.JSX;*.TXT");
}else{
	var mySavefile = File.saveDialog(nas.localize({en:"specify the file name for export",ja:"書き出しのファイル名を指定してください"}),"");
}
if(! mySavefile){return};
if(mySavefile.exists)
{
if(! confirm(nas.localize({en:"There is already a file with the same name.\nAre you sure you want to overwrite?",ja:"同名のファイルがすでにあります.\n上書きしてよろしいですか?"}))){return false;};
}
...
var myOpenfile = new File(mySavefile.fsName);
	...
	alert(nas.localize({en:"Extension Please specify js/jsx/txt ",ja:"拡張子は js/jsx/txt を指定してください。"}))
	return false;
};
}
// GUI Setup
...
{		testConsole.resultBox.addBuf(20);
		...
		testConsole.commandBox.text=nas.localize({
