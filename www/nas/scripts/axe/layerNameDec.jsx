...
var canStart = ((app.activeDocument.mode==DocumentMode.RGB)&&(! app.activeDocument.activeLayer.isBackgroundLayer));
	if(canStart){
        ...
        var myUndo="decrimentLayernames";
//関数版
var myAction='for(var ix=0;ix<myLayers.length;ix ++){myLayers[ix].name=nas.incrStr(myLayers[ix].name,-1,true);};activateByItemIndex(myLayers[0].itemIndex);if(myLayers.length>1){for(var ix=1;ix<myLayers.length;ix ++){manipulateByItemIndex(myLayers[ix].itemIndex,"add");};};';//
