 ...
 *	シートをプロジェクト内に保存する機能を実装中なのでイロイロ機能追加中　2009/10/10
 ...
 AE版との差異を調整
 新ライブラリ対応作業 2021 0126
 */

//	          *モジュール情報設定
var myFilename=("easyXPSLink_Ps.jsx");
var myFilerevision=("1.7");
var exFlag=true;
...
convertXps=function(datastream){
	if(! datastream.toString().length ){
        ...
        switch(true){
        case	(/^nasTIME-SHEET\ 0\.[1-9]/).test(datastream):;//	判定ルーチン内で先にXPSをチェックしておく（先抜け）
        break;
...
try{
	...
	alert(nas.localize(nas.uiMsg.dm000));//000:"nasライブラリが必要です。\nnasStartup.jsx を実行してください。"
	exFlag=false;
}
			}else{
...
//レイヤ別プロパティをストリームに追加
	    var Lprops=["option","link","id"];
	    for (var prp = 0 ; prp < Lprops.length ; prp ++){
		    var propName=Lprops[prp];
		    var lineHeader=(propName=="id")? 
		    nas.GUI.LineFeed+'[CELL\tN' : nas.GUI.LineFeed+'[' + propName + '\t';
		    XPSSummary+=lineHeader;
	        for (var i=0;i<XPS.xpsTracks.length;i++){
		        XPSSummary+="\t"+XPS["xpsTracks"][i][propName];
	        }
	        XPSSummary+='\t]';//
		}
        XPSSummary+="=============================================== memo:"+nas.GUI.LineFeed;
        XPSSummary+=XPS.xpsTracks.noteText.replace(/(\r\n?|\n)/g,nas.GUI.LineFeed);
    }
...
//セレクタの配列書き換え。
    ...
    for (var idx=0;idx<XPS.xpsTracks.length;idx++){newOptions.push(XPS.xpsTracks[idx].id);};
...
function guessLink(string){
    if(XPS.xpsTracks.length){
//BG/LO/をスキップ
	    if(string.match(/(^[-_].*|bg|lo|book)/i)){return 1;}

//検査(完全一致はやめた　冒頭一致で後ろの文字列は主に数値として許容)
	    for (Xid=0;Xid<XPS.xpsTracks.length;Xid++){
		    var Label=new RegExp("^"+XPS.xpsTracks[Xid].id+".*$","i");
...
//alert("Zero Length!");
		...
		if(XPS.readIN(myContent)){
		    ...
		    alert("XXX:"+XPS.errorMsg[XPS.errorCode]);return false;
		};
	}else{
//020:"読み込むタイムシートを選んでください"
		if(isWindows)
		{
			var mySheetFile = File.openDialog(nas.localize(nas.uiMsg.dm020),"timeSheetFile(*.xps;*.ard;*.tsh;*.sts;*.csv):*.XPS;*.ard;*.tsh;*.sts;*.csv");
		}else{
			var mySheetFile = File.openDialog(nas.localize(nas.uiMsg.dm020),"");
		}
         if (! mySheetFile){return false;};
		if (mySheetFile.name.match(/^[a-z_\-\#0-9]+\.(xps|ardj?|tsh|sts|csv|te?xt)$/i))
		{
			...
			if (mySheetFile.name.match(/\.sts$/i))
			{
			 ...
			 myContent = escape(myOpenfile.read());
         	 myOpenfile.close();
var myEncoding=GetEscapeCodeType(myContent);
var encTable={"SJIS":"CP932",
"EUCJP":"EUC-JP",
"JIS7":"ISO-2022-JP",
"JIS8":"JIS-0201",
"Unicode":"UTF16",
...
"UTF16LE":"UTF16-LE"}
			 ...
			 myOpenfile.open("r");
			 myOpenfile.encoding=encTable[myEncoding];
			 myContent = myOpenfile.read();
         	 myOpenfile.close();
...
switch(myEncoding){
    case "SJIS":
myContent=UnescapeSJIS(myContent);
    break;
    case "EUCJP":
myContent=UnescapeEUCJP(myContent);
    break;
    case "JIS7":
myContent=UnescapeJIS7(myContent);
    break;
    case "JIS8":
myContent=UnescapeJIS8(myContent);
    break;
    case "Unicode":
myContent=UnescapeUnicode(myContent);
    break;
    case "UTF7":
myContent=UnescapeUTF7(myContent);
    break;
    case "UTF16LE":
myContent=UnescapeUTF16LE(myContent);
    break;
    case "UTF8":
    default:
myContent=UnescapeUTF8(myContent);
}
//myContent=eval("Unescape"+myEncoding+"('"+myContent+"')");
*/
    }
			if(XPS.readIN(myContent)){
				...
				alert("BX"+XPS.errorMsg[XPS.errorCode]);
			};
	return true;
		}else {
//021:"タイムシートファイルを選択してください。"
	alert(nas.localize(nas.uiMsg.dm021));
	return false;
		};
	}
}
...
//シート書き出し
function saveSheet(){
if (! nas.easyXPS.sheetView.text){
	alert(nas.localize(nas.uiMsg.noSaveData));//"保存するデータがありません"
	return false;
}
if(isWindows)
{
//019:"書き出しのファイル名を指定してください"
	var mySavefile = File.saveDialog(nas.localize(nas.uiMsg.dm019),"nasXPSheet(*.xps):*.XPS");
}else{
...
{
//016:"同名のファイルがすでにあります.\n上書きしてよろしいですか?"
if(! confirm(nas.localize(nas.uiMsg.dm016))){return false;};
}
	...
	if(nas.XPSStore.getLength())
		{
			...
			if(confirm (nas.localize({en:"get data from XPSStore",ja:"XPSStoreからシートを取得します"}))){var myIndex=nas.XPSStore.pop();nas.otome.writeConsole("poped XPSStore:"+myIndex+" : "+nas.XPSStore.selected.name)};
			return;
		}
};
...
function pushSheet(){
	if(! isNaN(nas.XPSStore.getLength()))
	{
		...
		if(confirm (nas.localize({
			en:"It will write back the sheet to XPSStore.\nIt does nothing if there is no change",
			ja:"XPSStoreにシートを(書戻)保存します\n変更がなければ何もしません"
		}))){nas.XPSStore.push();}
		return;
	};
}
...
function checkLayer(){
	for (var idx=0;idx<thisProject.documents.length;idx++){
		...
		for(var Lid=1;Lid<=thisProject.documents[idx].layers.length;Lid++){
...
function initDocSelector_(){
	thisProject.documents=app.documents//選択対象のアイテムを全抽出
		...
		new_options.push(nas.localize({en:"<select a document>",ja:"<ドキュメント選択>"}));//国際化リソース注意
			for (var idx=0;idx<thisProject.documents.length;idx++){
					var myEntry="\[\x20"+idx+"\x20\]\x20\x20"+thisProject.documents[idx].name;
...
// window初期化
/*  CS6が動作異常を起こすので判定してダイアログに変更  */
if(parseInt(app.version,10)>13){
	nas.easyXPS = nas.GUI.newWindow("panel",nas.localize({
		en:"easy XPS Linker nas(u) tools (Nekomataya/2016)",
		ja:"仮設XPSリンカ nas(u) tools (Nekomataya/2016)"
	}),8,21,myLeft,myTop);
}else{
    nas.easyXPS = nas.GUI.newWindow("dialog",nas.localize({
		en:"easy XPS Linker nas(u) tools (Nekomataya/2016)",
		ja:"仮設XPSリンカ nas(u) tools (Nekomataya/2016)"
	}),8,21,myLeft,myTop);
}
// ファイルコントロール
	nas.easyXPS.loadButton=nas.GUI.addButton(nas.easyXPS,nas.localize(nas.uiMsg.Open),0,1,2,1);//"シート開く"
	nas.easyXPS.saveButton=nas.GUI.addButton(nas.easyXPS,nas.localize(nas.uiMsg.Save),2,1,2,1);//"シート保存"
...
//	nas.easyXPS.getCBButton=nas.GUI.addButton(nas.easyXPS,nas.localize({en:"getClipBoard",ja:""getClipBoard"}),0,2,2,1);
//	nas.easyXPS.putCBButton=nas.GUI.addButton(nas.easyXPS,nas.localize({en:"putClipBoard",ja:""putClipBoard"}),2,2,2,1);
	nas.easyXPS.readPjButton=nas.GUI.addButton(nas.easyXPS,nas.localize({en:"read Store",ja:"ストア読出"}),4,1,2,1);
	nas.easyXPS.writePjButton=nas.GUI.addButton(nas.easyXPS,nas.localize({en:"write Store",ja:"ストア書込"}),6,1,2,1);
//メッセージウェル
	nas.easyXPS.messageWell=nas.GUI.addStaticText(nas.easyXPS,nas.localize({
		en:"Reads the sheet, or paste it into the well",
		ja:"シートを読み込むか、ペーストしてください"
	}),0,2.3,4,0.8)
//シート更新ボタン
...
// シートセレクタ
	nas.easyXPS.XPSTLSelector=nas.GUI.addSelectButton(nas.easyXPS,"[　] <no-title>",0,0,10,8,1);

// ドキュメントセレクタ
nas.easyXPS.DocSelector=nas.GUI.addSelectButton(nas.easyXPS,nas.localize({en:"<select file>",ja:"<ファイル選択>"}),0,0,11,8,1);
// リンクブラウザ ダミーデータ
...
// リンクコマンドボタン
	nas.easyXPS.linkButton=nas.GUI.addButton(nas.easyXPS,nas.localize({en:"Apply sheet to the document",ja:"シートをドキュメントへ適用"}),0,18,4,1);
//	nas.easyXPS.readButton=nas.GUI.addButton(nas.easyXPS, nas.localize({en:"Create a sheet from the document",ja:"ドキュメントからシートを作成"}),4,18,4,1);
//フルフレームオプション
	nas.easyXPS.ffoButton=nas.GUI.addCheckBox(nas.easyXPS,nas.localize({en:"outputs all frames",ja:"全フレーム出力"}),4,18,4,1);

//ストア操作
	nas.easyXPS.addSheet=nas.GUI.addButton(nas.easyXPS,nas.localize({en:"add to Store",ja:"ストアに追加"}),0,19,2,1);
	nas.easyXPS.viewSheet=nas.GUI.addButton(nas.easyXPS,nas.localize({en:"Show all sheet",ja:"シート全表示"}),2,19,2,1);
	nas.easyXPS.removeSheet=nas.GUI.addButton(nas.easyXPS,nas.localize({en:"remove current sheet",ja:"カレントシート削除"}),4,19,2,1);
	nas.easyXPS.infoSheet=nas.GUI.addButton(nas.easyXPS,nas.localize({en:"sheet info",ja:"シート情報"}),6,19,2,1);
	
//	ファンクションボタン
	nas.easyXPS.cluButton=nas.GUI.addButton(nas.easyXPS,nas.localize({en:"Initialize buffer",ja:"バッファ初期化"}),0,20,2,1);
	nas.easyXPS.clbButton=nas.GUI.addButton(nas.easyXPS,nas.localize({en:"clear well",ja:"ウェル消去"}),2,20,2,1);
	nas.easyXPS.tstButton=nas.GUI.addButton(nas.easyXPS,nas.localize({en:"re-display",ja:"再表示"}),4,20,2,1);
	nas.easyXPS.closeButton=nas.GUI.addButton(nas.easyXPS,nas.localize(nas.uiMsg.Close),6,20,2,1);
...
//	ウィンドウ位置保存
	nas.easyXPS.onMove=function(){
        nas.GUI.winOffset["easyXPS"] =[ nas["easyXPS"].bounds[0],nas["easyXPS"].bounds[1]];
	}
...
if(app.documents.length){
	...
	for(var fidx=0;fidx<targetFolders.length;fidx++){
		...
		for(var fx = 0; fx < files.length ; fx ++){
			if(files[fx].name.match(/.*\.(xps|ard|tsh|sts)/i)){mySheets.push(files[fx])};
		};
		if(mySheets.length){
//シートあれば…なければスキップ
//読み込み対象シートを絞らずに全て読み込んでストアにaddする
			var currentXPSId=0
			for(var myIx = 0; myIx < mySheets.length ; myIx ++){
...
//	読み込み不良は未対処(07/04/02)
	...
	var msg=nas.localize(nas.uiMsg.noDocument);//"ドキュメントがありません"
	alert(msg);
}
