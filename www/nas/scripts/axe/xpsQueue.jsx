/**
 *	XpsオブジェクトからPsキューフレーム列を生成する
 *	
 */

var myXps=XPS;
var qFrames = [];//配列コレクション
/**
...
QFrame=function(myIndex,myBody,myDuration){
	...
	if(! myDuration) myDuration = 1;//０は禁止
//入力をフィルタしておく
...
/*-----------------------*/
//XPSから有効値で埋めた配列データをレイヤの数だけ取得
//仕様変更によりダイアログトラック・カメラワークトラックの値も取得する
	var tempArray=new Array();
	for(var lix=0;lix<myXps.xpsTracks.length;lix++){
...
//XPSのフレームを順次検査してユニークなキューフレーム配列を形成する
	...
	for(var fidx=0;fidx<myXps.duration();fidx++){
		...
		for(var lix=0;lix<myXps.layers.length;lix++){
			myOrderingArray.push(tempArray[lix][fidx]);
		};
