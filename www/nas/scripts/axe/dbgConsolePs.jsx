...
Axe 用改装
	このコンソールは、下段のテキストボックスの内容をeval()で実行して、
...
/*	ウィンドウにGUIパーツを配置	*/
nas.axe.dbgConsole.titleLabel=nas.axe.dbgConsole.add("statictext",nasGrid(0,0,480,24),"AXEconsole nas(u) tools (Nekomataya/2020)",{multiline:false});nas.axe.dbgConsole.titleLabel.justify="right";
...
//nas.axe.dbgConsole.btn02=nas.axe.dbgConsole.add("button",nasGrid(3,10,96,24),"app~");
	...
	nas.axe.dbgConsole.actButton.onClick = function (){try{nas.axe.dbgConsole.resultBox.text += eval(nas.axe.dbgConsole.commandBox.text.replace(/[‘’]/g,"'").replace(/[“”]/g,'"'))+LineFeed;}catch(err){nas.axe.dbgConsole.resultBox.text +=err.toString()+LineFeed;}};
...
if(nas.axe.dbgConsole.isDoc){
	...
	nas.axe.dbgConsole.onResize();
	nas.axe.dbgConsole.commandBox.text="/*\tcommand or expression \t"+LineFeed+"\tInput of the new line in the following key input"+LineFeed+"\t[ctlr]+[Enter]\t/or \t;\t[ctlr]+[M]\t/(old Mac)"+LineFeed+"\t[esc]\t/for exit"+LineFeed+" */"+LineFeed;
